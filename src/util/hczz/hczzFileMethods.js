...
export default {
  methods: {
    ...
    async uploadFiles(e,num) {
      ...
      if (fileList !== "") {
        this.hczzData.originFile = "附件已上传";
      }
      // 上传文件接口
      let data = await this.$Ajax.form(`upload/batch?classify=${num}`, formData);
      if (data.data.length > 0) {
        data.data.forEach(item => {
          this.hczzData.attachments.push(item);  // 需要上传的数据列表
    ...
    deleleFile(item, child) {
      ...
      this.$confirm("此操作将删除附件, 是否继续?", "提示", {
        ...
        type: "warning"
          ...
          this.hczzData.attachments.forEach((ele, index) => {
            if (ele.id === item.id) {
              this.hczzData.attachments.splice(index, 1);
            }
          });
