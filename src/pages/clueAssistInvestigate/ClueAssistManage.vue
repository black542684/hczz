    ...
    <main>
      ...
      <div class="table-content pubTable">
        <ul class="clearfix" v-for="(item,index) in listData" :key="index" v-show="!noData">
          ...
          <li :title="item.csName">{{ item.name }}</li>
          <li :title="item.csCode">{{ item.code }}</li>
...
export default {
  ...
  data() {
    return {
      url: {
        getListDataUrl: "clue/invList",    //数据列表
        getNumUrl: "clue/invListTotal",    //线索协查总数
        getUserInfosUrl: 'userApi/getUserLoginInfoByToken',
        getStatusListUrl: 'clue/getStatusList'   //获取状态
      },
      params: {
        createBy: '',
        pageSize: 10,
  ...
  methods: {
    ...
    //  异步请求改成同步请求
    async getStatusList () {
      const data = await this.$Ajax.get(this.url.getStatusListUrl, {});
      this.statusList = data.data
      this.statusList.unshift({ name: '草稿', id: '0' })
    },
    // 获取用户信息
    async getUserInfos() {
      let data = await this.$Ajax.get(this.url.getUserInfosUrl, {});
      ...
      if (data.account !== '') {
        ...
        this.params.createBy = data.account;   // 当前登入的用户
        this.getListData();
        this.getNum()
      }
    },
    ...
    async getListData() {
      ...
      console.time('获取列表数据');
      let data = await this.$Ajax.get(this.url.getListDataUrl, this.params);
      console.log('列表数据',data);
      console.timeEnd('获取列表数据');
    ...
    getNum() {
        ...
        this.$Ajax.get(this.url.getNumUrl, params).then(data => {
          this.stateTotal = data.data;
        });
    },
    query() {
        if (
