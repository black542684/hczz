            ...
            <el-row :gutter="20" class="el-row-style">
                <el-col :span="6" class="el-col-style">
                    <div class="grid-content">
                        <span class="header-title">研判指令详情</span>
            ...
            <el-form 
                ...
                <div class="form-item" style="padding-bottom: 20px;">
                    ...
                    <el-form-item label="签收单位：">
                        <div>{{ affirmInfos.depName }}</div>
                    </el-form-item>
                    <el-form-item label="签收人：">
                        <p class="success-result">{{ affirmInfos.createName }}</p>  
                    </el-form-item>
                    <!-- 占位 -->
                    <el-form-item></el-form-item>
                    <el-form-item 
...
export default {
    ...
    mounted(){
        ...
        //  获取数据
        this.$Ajax.get('fightcmd/cmdDetails',{id:this.bkID,createBy:"shenhe"}, true).then(res=>{
            console.log(res);
            //  指令信息
            ...
            res.data.activitiInfoList.data.forEach(item => {
                ...
                if (item.endTime !== null & item.taskDefinitionKey === 'qrpj') {
                    this.affirmInfos = item.hParam
                }
            });
    ...
    methods:{
        ...
        //  文件上传模块
        beforeRemove(file, fileList) {          //  文件移除提示
            return this.$confirm(`确定移除 ${ file.name }？`);
        },
        uploadSuccess(res, file){        //  文件上传成功钩子
            this.isUpload = true;
        },
        removeSuccess(file, fileList){      //  文件全部移除钩子
            this.isUpload = false;
        },
