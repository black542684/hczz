...
offmapcfg.home = "static/offlinemap/";
(function(){
	...
	*/
	...
	Zc.getTilesUrl = function(a, b, c) {
       ...
       // my_log('普通地图:' + tdir);
        ...
        // my_log('普通地图:' + Xc[Math.abs(d + a) % Xc.length]);
        my_log('普通地图:' +  (Xc[Math.abs(d + a) % Xc.length] + "?qt=tile&x=" + (d + "").replace(/-/gi, "M") + "&y=" + (a + "").replace(/-/gi, "M") + "&z=" + b + "&styles=" + c + "&scaler=" + f + (6 == x.da.la ? "&color_dep=32&colors=50" : "") + "&udt=" + e).replace(/-(\d+)/gi, "M$1"));
        // http://192.168.172.149:8088/mts/getTile?x=191&y=67&z=10
		// http://192.168.172.130:8088/mts/getTile?x=194&y=71&z=10
        var local_map = window.g.mapApi + "/mts/getTile?" + "x=" + (d + "").replace(/-/gi, "M") + "&y=" + (a + "").replace(/-/gi, "M") + "&z=" + b ; // x=194&y=71&z=10
      my_log('普通地图:' + local_map);
      return local_map;
	...
	var Ma = new Vc("\u5730\u56fe", Zc, {
		...
		$c = new Hc;
	$c.oO = [z.url.proto + z.url.domain.TIlE_PERSPECT_URLS[0] + "/resourcesManage/mappic/", z.url.proto + z.url.domain.TIlE_PERSPECT_URLS[1] + "/resourcesManage/mappic/", z.url.proto + z.url.domain.TIlE_PERSPECT_URLS[2] + "/resourcesManage/mappic/", z.url.proto + z.url.domain.TIlE_PERSPECT_URLS[3] + "/resourcesManage/mappic/"];
